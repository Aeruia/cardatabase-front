...
class CarList extends Component {
    ...
    onDelClick = (link) => {
        if (window.confirm('Are you sure to delete?')) {
            ...
            fetch(link, { method: 'DELETE', headers: { 'Authorization': token } })
                .then(res => { toast.success("Car deleted", { position: toast.POSITION.BOTTOM_LEFT }); this.fetchCars() })
                .catch(err => { toast.error("Error when deleting", { position: toast.POSITION.BOTTOM_LEFT }); console.log(err) });
        }
    }
    ...
    addCar(car) {
        ...
        }).then(res => this.fetchCars()).catch(err => console.log(err))
    }
